...
class PlanModel {
    ...
    /** How many CP-SAT solves the last [solve] required. */
    fun solvesInLastRequest(): Int = solveCount
    ...
    private fun <T> timed(kind: String, solve: () -> T): T {
        // The timer this used to record went to a MeterRegistry that does not
        // exist in this process -- there is no Spring here, and a metrics backend
        // in a JVM that lives for one solve would have nowhere to publish to. The
        // count survives because it is the load-bearing half: the endpoint's
        // twenty-second worst case is this number times the five-second cap, and
        // `PlanSolverBoundTest` asserts it rather than timing it.
